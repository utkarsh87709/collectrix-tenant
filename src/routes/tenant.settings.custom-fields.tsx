...
import {
  Plus, Trash2, Pencil, X, Type, Hash, Calendar as CalIcon, DollarSign,
  List, CheckSquare, ToggleLeft, Upload, EyeOff,
} from "lucide-react";
import { toast } from "sonner";
import {
  useCustomFields, addCustomField, updateCustomField, deleteCustomField,
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {fields.map((f) => {
                  ...
                  return (
                    <tr key={f.id} className="border-t border-border">
                      ...
                      <td className="px-3 py-3">{f.required ? <Pill tone="warning">required</Pill> : <span className="text-xs text-muted-foreground">optional</span>}</td>
                      ...
                      <td className="px-3 py-3 text-right">
                        <div className="inline-flex items-center gap-1">
                          <button onClick={() => setEditing(f)} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground hover:text-tenant"><Pencil className="h-3.5 w-3.5" /></button>
                          <button
                            onClick={() => {
                ...
                {fields.length === 0 && (
                  <tr><td colSpan={7} className="px-6 py-12 text-center text-sm text-muted-foreground">No custom fields yet — click "New custom field" to add one.</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function FieldEditor({
  ...
  const submit = () => {
    ...
    const finalKey = (key || label).toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "");
    onSave({
      label: label.trim(),
      key: finalKey,
      type,
      ...
      visibleToRoles,
      options: (type === "dropdown" || type === "multiselect")
    ...
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl border border-border shadow-elegant w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="font-display text-lg font-bold">{initial ? "Edit custom field" : "New custom field"}</h2>
          <button onClick={onClose} className="p-1 rounded-md hover:bg-muted"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Label">
              <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. Court jurisdiction" className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm" />
            </FormField>
            <FormField label="Storage key">
              <input value={key} onChange={(e) => setKey(e.target.value)} placeholder="auto from label" className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm font-mono" />
            </FormField>
          </div>
          ...
          {(type === "dropdown" || type === "multiselect") && (
            <FormField label="Options (one per line)">
              <textarea value={optionsText} onChange={(e) => setOptionsText(e.target.value)} rows={4} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm font-mono" placeholder={"Option A\nOption B\nOption C"} />
            </FormField>
          )}
              ...
              {ROLES.map((r) => {
                ...
                return (
                  <button
                    key={r}
                    type="button"
                    onClick={() => setVisibleToRoles((p) => on ? p.filter((x) => x !== r) : [...p, r])}
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Import mapping (CSV header)">
              <input value={importMapping} onChange={(e) => setImportMapping(e.target.value)} placeholder="e.g. court_jurisdiction" className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm font-mono" />
            </FormField>
            <FormField label="CRM mapping">
              <input value={crmMapping} onChange={(e) => setCrmMapping(e.target.value)} placeholder="e.g. Debtor.CustomField1" className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm font-mono" />
            </FormField>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-border">
            ...
            <Toggle label="Use in duplicate detection" value={useInDedupe} onChange={setUseInDedupe} />
            <Toggle label="Use in verification" value={useInVerification} onChange={setUseInVerification} />
            <Toggle label="Use in reporting" value={useInReporting} onChange={setUseInReporting} />
          </div>
        </div>
        <div className="px-6 py-4 border-t border-border flex items-center justify-end gap-2">
          <button onClick={onClose} className="px-3 py-2 rounded-lg border border-border text-sm font-semibold hover:bg-muted">Cancel</button>
          <button onClick={submit} className="px-4 py-2 rounded-lg bg-gradient-tenant text-white text-sm font-semibold shadow-tenant">
  ...
  return (
    <label className="flex items-center gap-2 cursor-pointer py-1">
      <span className="relative inline-flex items-center">
        ...
        <span className="w-9 h-5 bg-muted rounded-full peer-checked:bg-tenant after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition peer-checked:after:translate-x-4 border border-border" />
      </span>
