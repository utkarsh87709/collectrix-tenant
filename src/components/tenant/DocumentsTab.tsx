...
const SOURCE_META: Record<DocSource, { label: string; icon: typeof FileText; tone: "tenant" | "warning" | "success" | "info" | "muted" }> = {
  ...
  debtor: { label: "Debtor", icon: Paperclip, tone: "tenant" },
};
...
function seedFor(debtorId: string): DebtorDoc[] {
  return [
    ...
    { id: `${debtorId}-d3`, name: "Debtor_ID_Verification.jpg", stage: "validation", source: "debtor", uploadedBy: "Debtor portal", uploadedAt: "38d ago", size: "1.2 MB", mime: "image" },
...
function GenerateDialog({
  ...
  function runGeneration() {
    ...
    setTimeout(() => {
      ...
      const body = mergeTemplate(template.preview, ctx) + `\n\nThis document was generated by Lovable AI from the "${template.name}" template using the debtor's current profile, balance, and account history.\n\nSincerely,\n${ctx.attorney_name}\n${ctx.law_firm_name}\n${ctx.firm_phone}`;
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Select a template — Lovable AI will merge {debtorName}'s details, balance, and account context to produce a ready-to-send document.
        </p>
            ...
            <div className="bg-background rounded-lg border border-border p-4 min-h-[180px] max-h-[260px] overflow-y-auto text-xs font-serif whitespace-pre-wrap leading-relaxed">
              {generating && (
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Loader2 className="h-4 w-4 animate-spin" /> AI is drafting the document with the debtor's details…
                </div>
              )}
